...
A Raspberry Pi on the same home network acts as the bridge. It runs a minimal Flask server exposing a single `GET /reboot` endpoint. When called, the Pi connects to the Firestick over ADB and triggers a reboot, the equivalent of running this locally:
